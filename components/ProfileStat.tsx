...
import { LikedList } from './stats/LikedList';
...
export const ProfileStat = () => {
    ...
    return (
        ...
        <div>
            ...
            <Header title={"Tracks You Liked in Past 6 Months"}></Header>
            </div>
            <div>
            <div>
                {likedList.map((song: any, index: number) => (
                    <LikedList key={index} title={song.track_name} artist={song.track_artist} album={song.track_album} rYear={song.track_release_year}></LikedList>
                ))}
            </div>
        </div>
        </div>
